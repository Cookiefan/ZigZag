...
}_data[N], data[N];   //_data[]数组代表输入的数据
...
int Build(int l, int r, int dept)
{
    ...
    data[now].lc=Build(l, mid - 1, dept + 1);  //递归左子树
    data[now].rc=Build(mid + 1, r, dept + 1);  //递归右子树
}
...
void KDTree::Query(Node *rt, Node p, int m, int dept)
{
    ...
    Node* x = rt->lc;
   	Node* y = rt->rc;
    if(pfeature[dim] >= rt->feature[dim]) //数据p的第dim个特征值大于等于当前的数据，则需要进入右子树
        swap(x, y);
